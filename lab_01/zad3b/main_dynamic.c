...
int main(int argc, char **argv){
    ...
    operationBlock** (*createBlockArray)();
    createBlockArray = (operationBlock** (*)())dlsym(handle,"createBlockArray");
    if(dlerror() != NULL){ printf("Can't load function"); exit(-1);}

    void (*compare)();
    compare = (void (*)())dlsym(handle,"compare");
    if(dlerror() != NULL){ printf("Can't load function"); exit(-1);}

    int (*createBlock)();
    createBlock = (int (*)())dlsym(handle,"createBlock");
    if(dlerror() != NULL){ printf("Can't load function"); exit(-1);}

    int (*deleteBlock)();
    deleteBlock = (int (*)())dlsym(handle,"deleteBlock");
    if(dlerror() != NULL){ printf("Can't load function"); exit(-1);}

    int (*deleteOperation)();
    deleteOperation = (int (*)())dlsym(handle,"deleteOperation");
    if(dlerror() != NULL){ printf("Can't load function"); exit(-1);}

    filePair* (*makePairs)();
    makePairs = (filePair* (*)())dlsym(handle,"makePairs");
    if(dlerror() != NULL){ printf("Can't load function"); exit(-1);}
