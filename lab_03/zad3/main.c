...
void process(char* filename, int id, int workers, int max_time, int time_lim, int mem_lim){
    struct rlimit lim; lim.rlim_cur = time_lim; lim.rlim_max = time_lim;
    setrlimit(RLIMIT_CPU, &lim);
    lim.rlim_cur = lim.rlim_max = mem_lim * 1024 * 1024;
...
int main(int argc, char **argv){
    ...
    int time_lim = atoi(argv[4]);
    int mem_lim = atoi(argv[5]);

    for_i_up_to(num_processes){
       pid_t child = fork();
       if(child==0){
           process(file, i, num_processes, max_sec, time_lim, mem_lim);
       }
    }
